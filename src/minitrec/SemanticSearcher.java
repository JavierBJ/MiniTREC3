...
public class SemanticSearcher {
	...
	public static void main(String[] args) {
        ...
        queryStrings[2] = "PREFIX rdf: <" + RDF.getURI() + "> PREFIX rdfs: <" + RDFS.getURI() + "> PREFIX mt: <http://minitrec/> " +
        		"PREFIX im: <http://imred.es/> PREFIX dc: <http://purl.org/dc/elements/1.1/> " + 
        		"PREFIX sk: <http://www.w3.org/2004/02/skos/core#> PREFIX foaf: <"+FOAF.getURI()+"> " +
        		"PREFIX xsd: <http://www.w3.org/2001/XMLSchema#> " +
        		"SELECT distinct ?x WHERE { ?x rdf:type ?tipo . ?tipo rdfs:subClassOf mt:documento . " +
        		"{?x mt:subject im:guerraIndependencia} UNION {?x mt:subject im:historiaEspana}" +
        		"UNION {?x mt:subject ?relacionado . ?relacionado sk:related|sk:broader im:guerraIndependencia} " +
        		"UNION {?x mt:subject ?relacionado . ?relacionado sk:related|sk:broader im:historiaEspana}}";
        
        queryStrings[3] = "";
        
        queryStrings[4] = "";
        
        for (int i=0; i<3; i++) {	//CAMBIAR i<2 cuando metas mas consultas
